...
def correlation_plots(z_test, p_val, test_name, var_names):
    ...
    # Local variables
    nvar = len(var_names)
    pval = 1 - (p_val + np.eye(nvar))  # Transformation convenient for plotting below
    ...
    plt.colorbar()
    plt.savefig('plots/' + test_name + '_cross_correlation.png')
    ...
    plt.savefig('plots/' + test_name + '_significance.png')
    plt.clf()
